...
import com.Gestion.MiBalnearioGestion.Productos.Mapper.ProductoMapper;
...
public class ProductoService  implements IProductoService {
    ...
    public void borrar (UUID publicId)
    {
        ProductoEntity buscado = productoRepository.
                ...
                .orElseThrow(()-> new EntidadNoEncontradaException("El producto no se encontro  con id: "+ publicId.toString(), "ProductoEntity"));

        if (!buscado.getProductoDisponible()) {
            throw new DatosInvalidoException("El producto ya se encuentra dado de baja","ProductoEntity");
        }
    ...
    public ProductoDTO actualziar (UUID publicId, ProductoDTO dto) {
        ProductoEntity producto = productoRepository
                .findByPublicId(publicId)
                .orElseThrow(() -> new EntidadNoEncontradaException(" El Producto no se encontro : ", dto.toString()));
    ...
    public void reactivar(UUID publicId) {
        ProductoEntity buscado = productoRepository.findByPublicId(publicId)
                .orElseThrow(() -> new EntidadNoEncontradaException("El producto no se encontró con id: ", publicId.toString()));
        if (buscado.getProductoDisponible()) {
            throw new DatosInvalidoException("El producto ya se encuentra activo y disponible", "ProductoEntity");
        }
