...
public interface IPagoRepository extends JpaRepository<PagoEntity, Long>, JpaSpecificationExecutor<PagoEntity> {
    ...
    @Query("SELECT p FROM PagoEntity p WHERE TYPE(p) = PagoPedidoLugarEntity " +
            "AND TREAT(p AS PagoPedidoLugarEntity).pedido.publicId = :publicId")
    Optional<PagoEntity> findPagoPorPedidoLugar(@Param("publicId") UUID publicId);
}
