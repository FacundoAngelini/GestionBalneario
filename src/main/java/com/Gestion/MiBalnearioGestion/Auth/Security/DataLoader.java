...
public class DataLoader implements CommandLineRunner {
    ...
    private final ConfgTemporadaRepository configTemporadaRepository;
        ...
        public void run(String... args) throws Exception {
            ...
            // 2. ROLES DE NEGOCIO PARA EMPLEADOS
            ...
            // 4. ADMINISTRADOR SUPREMO (CORREGIDO)
            if (!credentialsRepository.existsByNombreUsuario("admin_supremo")) {

                // 4.a Usuario base vacío y activo
                ...
                CredencialEntity credencialAdmin = CredencialEntity.builder()
                        ...
                        .roles(new HashSet<>(Set.of(rolAdminDb)))
                        .build();

                // 4.c Generamos y guardamos el token
                String tokenInicial = jwtService.generateRefreshToken(credencialAdmin);
                credencialAdmin.setRefreshToken(tokenInicial);
                credentialsRepository.save(credencialAdmin);

                System.out.println("Administrador supremo creado: admin_supremo / admin123");
            }
            ...
            // 6. CLIENTE DE PRUEBA COMPLETO (CORREGIDO)
            ...
            if (!clienteRepository.findByPublicId(clienteIdPrueba).isPresent()) {
                ...
                // 6.b Buscamos el rol de seguridad
                ...
                // 6.c Creamos la credencial para que pueda hacer login
                CredencialEntity credencialCliente = CredencialEntity.builder()
                        .nombreUsuario("juan_cliente_prueba")
                        .contrasenia(passwordEncoder.encode("cliente123"))
                        .enabled(true)
                        .usuario(usuarioCliente)
                        .roles(new HashSet<>(Set.of(rolClienteDb)))
                        .build();

                String tokenInicialCliente = jwtService.generateRefreshToken(credencialCliente);
                credencialCliente.setRefreshToken(tokenInicialCliente);
                credentialsRepository.save(credencialCliente);

                // 6.d Creamos la entidad Cliente y la vinculamos al Usuario
                ...
                System.out.println("Cliente de prueba indexado correctamente: juan_cliente_prueba / cliente123");
            }

            // 7. RECURSO (CARPA) CON TARIFA VIGENTE
